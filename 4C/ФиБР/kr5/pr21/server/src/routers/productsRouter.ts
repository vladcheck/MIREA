...
import invalidateProductsCache from "../utils/cache/invalidateProductsCache";
  ...
  .get(
    ...
    async (req: CacheMiddlewareRequest, res: Response) => {
      ...
      const filteredEntries = entries.filter((p) => {
        return queryParams.author_id ? p.author_id === queryParams.author_id : true;
      });
      ...
      if (req.cacheKey && req.cacheKey?.length > 0) {
        await saveToCache(req.cacheKey, filteredEntries, req.cacheTTL);
      }
    ...
    async (req: Request, res: Response) => {
      ...
      if (Number.isNaN(parseFloat(body.price)) || parseFloat(body.price) < 0) {
        return res
          ...
          .send(getErrorString(`Некорректная цена`, req.body.price, `неотрицательное число`));
      }

      const files = req.files as Express.Multer.File[];
      const uploadedImages = files?.map((f) => `/api/public/images/products/${f.filename}`) || [];
      ...
      try {
        ...
        return res.status(StatusCodes.CREATED).json(newProduct).send(ReasonPhrases.CREATED);
      } catch (error) {
        console.error(error);
...
productsRouter
  .get(
    "/:id",
    cacheMiddleware((req: Request) => `products:${req.params["id"]}`, PRODUCTS_CACHE_TTL),
    async (req: CacheMiddlewareRequest, res: Response) => {
  ...
  .post(
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        await dbFacade.updateEntryById(productsPath, p.id, p);
      } catch (error) {
        console.error(error);
        return getInternalServerError(res, error);
      }
